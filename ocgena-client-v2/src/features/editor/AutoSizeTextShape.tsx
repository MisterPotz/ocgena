...
function getSizeBeautifiedText(text: Konva.Text, wishedText: string) {
  ...
  const originalTextLines = previousTextLines
  ...
  while (diffToIdeal(currentRelation) <= diffToIdeal(previousIteration)) {
    ...
    const currentLineMetrics = calculateTextLinesMetrics(text, currentTextLines)
    console.log(
      "currentLineMetrics,",
      currentLineMetrics,
      "text",
    ...
    const tempCurrentRelation = currentRelation
...
export function AutoSizeTextShape({ element, updatePosition }: TextShapeProps) {
  ...
  useEffect(() => {
    if (element.text && element.text.trim().length > 0) {
      const lines = getSizeBeautifiedText(textRef.current!, element.text)
  ...
  useEffect(() => {
    if (selectedAtClick) {
      if (trRef.current && groupRef.current) {
        const transformableNodes = [groupRef.current!]
...
const setupRemovableTextArea = (
  ...
  groupNode.on("dblclick dbltap", (e: KonvaEventObject<MouseEvent>) => {
    ...
    const escapeListener = createEscapeListener(removeTextArea)
